...
class _BASIC_DLL_API CBasicCombinFile : public basiclib::CBasicFileObj
{
public:
	CBasicCombinFile(void);
	virtual ~CBasicCombinFile(void);
public:
	// 文件操作
	//! \brief 打开一个zip文件。这个操作过程中应该已经读取所有的文件索引
	virtual long	OpenCombinFile(const char* lpszFileName, DWORD dwOpenFlags);
	//! \brief 关闭
	virtual void	Close();
	//! \breif 写盘
	virtual long	Flush();
	...
	//!子文件的操作
	bool GetPackFile(const char* filename, basiclib::CBasicSmartBuffer& smBuf);
	//! \brief 得到文件信息
	bool	GetPackFileStatus(const char* filename, BasicPackFileStatus& status);

	//! \brief 增加一个文件,如果dir为空，则直接取fileobj的文件名.
	// 如果该文件名已经存在则替换已经存在的文件。
	bool	AddPackFile(const void* pBuffer, size_t length, const char* filename);
	bool	AddPackFile(basiclib::CBasicStaticBuffer* buffer, const char* filename);
	...
	//readfromfile
	bool ReadFileFromFileInfo(const basicfile_info* info, basiclib::CBasicSmartBuffer& smBuf);
	...
	class ShortenOffset;
	friend class BasicExtractor;
protected:
	virtual long	buildZipIndex();

	bool			extractToFile(const char* dir, const basicbase_file_info* info);
	bool			setFileTime(const char* filename, time_t mtime);

	bool			repairFile();

	static	basicbase_file_info* deleteFileInfo(const char* file, basicfile_container* dir);

	static	void		getFileStatus(const basicbase_file_info* info, BasicPackFileStatus& status);
	static	bool		addFile(const char *filename, basicbase_file_info* file, basicfile_container* dir);

	static  basiclib::CBasicString getRootPath(const char* fileame);
	static  const char* getFileName(const char* filename);
	static  const char* stepPath(const char* filename);
protected:
	//checkfunc
	bool CheckFuncImpl(basicfile_container& files);
public:
	basicfile_container		m_Files;
	basicfile_end*			m_pEnd;
	bool					m_bNeedRepair;
};

///////////////////////////////////////////////////////////////////////////////////////////////
class CBasicPackCombinFile : public CBasicCombinFile
{
public:
	CBasicPackCombinFile();
	virtual ~CBasicPackCombinFile();

	//初始化函数
	long InitPackComBinFile(const char* pRelocatePath, const char* lpszFileName);

	//获取文件,传入绝对位置
	bool GetFileByName(const char* pFile, basiclib::CBasicSmartBuffer& smBuf);
	//获取文件，传入相对路径
	bool GetFileByRelocateName(const char* pFile, basiclib::CBasicSmartBuffer& smBuf);

	//合并文件
	bool CombinFile(CBasicCombinFile& combinFile);
protected:
	basiclib::CBasicString m_strPathRelocate;
};
//全局公用的
typedef basiclib::CBasicSingleton<CBasicPackCombinFile> SingletonString_CBasicPackCombinFile;
/////////////////////////////////////////////////////////////////////////////
