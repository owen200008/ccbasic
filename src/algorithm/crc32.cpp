...
uint32_t Basic_crc32_Init()
{
	return -1;
}

uint32_t Basic_crc32_Update(Net_UInt oldCrc32, unsigned char* pszData, unsigned int nDataLength)
{
	register Net_UInt crc = oldCrc32;
	register unsigned char *cp = pszData;
	register unsigned int cnt = nDataLength;

	while (cnt--)
	{
		crc = (crc >> 8)^crc32tab[(crc & 0xff) ^ *cp++];
	}

	return crc;
}

uint32_t Basic_crec32_End(Net_UInt oldCrc32)
{
	oldCrc32 ^= 0xffffffff;
	return oldCrc32;
}

uint32_t Basic_crc32(unsigned char* pszData, unsigned int nDataLength)
{
	uint32_t crc = Basic_crc32_Init();
	crc = Basic_crc32_Update(crc, pszData, nDataLength);	
...
long Basic_crc32_File(const char* lpszFileName, uint32_t &crc){
	...
	long lReadCount = 1024;
	long lTotalSize = file.GetLength();
